...
class Building{
	private:
		int N; //visitors' capacity in the building 
		int no_visitors; //current number of visitors in building
		Floor* f1;
		Floor* f2;
		Floor* f3;
		Floor* f4;
	...
	public:
		...
		//Increase the number of visitors the current time in building
		...
		int Enter(){
			if(no_visitors < N){//visitor can join the building
				if ((this->gf)->Enter() ==1 ){
